...
int main(int argc, char** argv) {
    ...
    param.providers[1].deviceType = 0; // set 1 if using CUDA for sam, set 0 if using cpu for sam
    Sam sam(param);
    ...
    cv::Mat maskAuto = sam.autoSegment({ 10, 10 }); // .onnx requires image to be 1024*720
    cv::imwrite("sam_ckpt/output-auto.png", maskAuto);
    return 1;
}
...
/*
  ...
  if (FLAGS_h) {
    std::cout << "Example: ./sam_cpp_test -pre_model=\"sam_ckpt/sam_preprocess.onnx\" "
                 "-sam_model=\"sam_ckpt/sam_vit_h_4b8939.onnx\" "
                 "-image=\"sam_ckpt/test_input.jpg\" -pre_device=\"cpu\" -sam_device=\"cpu\""
              << std::endl;
    return 0;
  }
